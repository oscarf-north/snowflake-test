CREATE OR REPLACE PROCEDURE PRD_HOSPENG_REPORTING.GITHUB.EXPORT_PROCEDURES_DDLS("DB" VARCHAR, "TARGET_SCHEMA" VARCHAR)
RETURNS VARCHAR
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
...
snowflake.createStatement({ sqlText: `TRUNCATE TABLE ${TARGET_SCHEMA}.export_ddls_sp` }).execute();
...
while (rsSchemas.next()) {
    ...
    while (rsProcs.next()) {
        ...
        try {
            ...
            var insert = `
                INSERT INTO ${TARGET_SCHEMA}.export_ddls_sp (object_type, schema_name, object_name, ddl)
